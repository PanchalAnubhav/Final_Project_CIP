...
Personal Finance Tracker is a GUI-based expense management tool that lets users log spending, visualize category breakdowns, and review transaction history — all from a clean, terminal-style interface. Data is saved locally as JSON, so records persist across sessions. The storage layer is intentionally structured to mirror a document database schema, making a future MongoDB migration a near drop-in replacement — with each transaction object mapping directly to a MongoDB document and each spending category mapping to a collection.
...
- **NoSQL-ready schema design** — JSON structure mirrors MongoDB documents for a seamless future migration
---
 
## Future Roadmap
 
- **MongoDB integration** — swap the JSON file layer for `pymongo`, enabling multi-device sync, date/amount range queries, and cloud persistence with zero changes to the UI or business logic
- **Web version** — expose the data layer via a FastAPI backend, pairing naturally with the existing document-style schema
- **Budget limits** — set per-category spending caps with alerts when thresholds are exceeded
- **Export to CSV** — one-click export of transaction history for use in spreadsheets
---
